...
В разделе **«Администрирование»** на вкладке **«Инструкции»** вы можете редактировать текст инструкций для каждой роли.`
    ...
    TRIMMER: {
        title: 'Инструкция Обрубщика',
        content: `## 1. Мои этапы\nВ разделе **«Мои Задачи / Этапы»** отображаются партии, направленные на обрубку.\n\n## 2. Работа и сдача\nНажмите **«Начать работу»**, чтобы закрепить этап за собой. После завершения операции нажмите **«Завершить этап»** и укажите количество годных и забракованных изделий.`
    },
    MOULDER: {
        title: 'Инструкция Формовщика',
        content: `## 1. Изготовление форм\nФормуйте изделия согласно сменному заданию.\n\n## 2. Сдача этапа\nОтмечайте в системе изготовленные формы, указывая годные и брак, чтобы передать их на следующий этап (Заливка).`
    },
    POURER: {
        title: 'Инструкция Заливщика',
        content: `## 1. Заливка металла\nПринимайте подготовленные формы (или кокили) в работу.\n\n## 2. Завершение\nУказывайте параметры заливки и завершайте этап для передачи на выбивку/обрубку.`
    },
    KNOCKER: {
        title: 'Инструкция Выбивщика',
        content: `## 1. Выбивка\nПроизводите выбивку форм после заливки.\n\n## 2. Сдача\nПередавайте выбитые отливки на обрубку через систему, указывая количество годных.`
    },
    FINISHER: {
        title: 'Инструкция Доработчика',
        content: `## 1. Доработка\nВыполняйте финишную обработку (зачистка, сверловка и т.д.) перед сдачей в ОТК.\n\n## 2. Завершение\nСдавайте годные изделия контролеру ОТК через личный кабинет.`
    },
    DEMO: {
        title: 'Инструкция Демо-пользователя',
        content: `## Добро пожаловать в демо-режим LITEXAL ERP!\n\nВы находитесь в режиме ознакомления. В этом режиме вам доступны **полные копии** реальных рабочих столов и модулей системы, но они заполнены **демонстрационными (сгенерированными) данными**.\n\n## Как посмотреть другой модуль?\n\nВ самом верху экрана (в шапке сайта) есть выпадающий список **«Вид»**. Нажмите на него и выберите любую должность (например, Администратор, Мастер, Технолог, ОТК). Система моментально переключит вас в интерфейс выбранной роли, и вы увидите функционал глазами этого сотрудника.\n\n## Обратите внимание:\n\n*   **Данные не сохраняются:** Вы можете нажимать любые кнопки, пробовать оформлять заказы или выпускать партии, но при обновлении страницы все данные вернутся в исходное демонстрационное состояние.\n*   **Безопасность:** Реальные производственные данные в этом режиме полностью скрыты и недоступны.\n*   **Тур по системе:** Нажмите на желтую кнопку **«Тур»** вверху экрана, чтобы заново посмотреть основные возможности системы.`
    }
};
